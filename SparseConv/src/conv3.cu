...
void _spmm_conv_0(const float * __restrict__ input_data, float *output_data, const int ptr_start, const int ptr_end, const int * __restrict__ kernel_ptr_all, const int * __restrict__ kernel_map_all, const int * __restrict__ kernel_offset, const float * __restrict__ kernel_data) {
	...
	int i = (threadIdx.y * 25) + blockIdx.x * (25 << 2);
	...
	int x1 = output_x * 1 * 5 * 16 * 32 + output_y * 1 * 16 * 32 + c;

	float res[25<<1];
#pragma unroll
	for (int i=0; i<(25<<1); i++) res[i] = 0.0f;

	int kernel_off;
	float kernel_value[25];
	int begin = 0;
	...
	for (int b=start; b<interm1; b+=8) {
		if (((b - start) & 31) == 0) {
			...
			if (threadIdx.x < end - b) {
				kernel_off = x1 + kernel_offset[threadIdx.x+b] / (16 * 5)  *5 * 16 * 32 + kernel_offset[threadIdx.x+b] / 16 % 5 * 16 * 32   + kernel_offset[threadIdx.x+b] % 16 * 32;
#pragma unroll
				for (int k=0; k<25; k++) {
...
#pragma unroll
	...
	if (interm1 < end && ((interm1-start)  & 31) == 0) {
		...
		if (threadIdx.x < end - interm1) {
			kernel_off = x1 + kernel_offset[threadIdx.x+interm1] / (16 * 5)  *5 * 16 * 32 + kernel_offset[threadIdx.x+interm1] / 16 % 5 * 16 * 32   + kernel_offset[threadIdx.x+interm1] % 16 * 32;
#pragma unroll
			for (int k=0; k<25; k++) {
...
#pragma unroll
	...
	int output_idx = (output_x*1*120+output_y*120)*32 + c;
#pragma unroll
	for (int k=0; k<25; k++) {
...
_spmm_conv_0<<<nblocks_0, nthreads_0, 0, stream_0>>>(input_data, output_data, 0, 26, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_1);
dim3 nblocks_1(0, 0);
dim3 nthreads_1(32, 4);
_spmm_conv_1<<<nblocks_1, nthreads_1, 0, stream_1>>>(input_data, output_data, 26, 51, kernel_ptr, kernel_map, kernel_offset, kernel_data);
cudaStreamCreate(&stream_2);
dim3 nblocks_2(0, 0);
dim3 nthreads_2(32, 4);
_spmm_conv_2<<<nblocks_2, nthreads_2, 0, stream_2>>>(input_data, output_data, 51, 74, kernel_ptr, kernel_map, kernel_offset, kernel_data);
